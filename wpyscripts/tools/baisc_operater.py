...
def screen_shot_click_pos(pos_x, pos_y, sleeptime=2, exception=True):
    ...
    try:
        report.capture_and_mark(pos_x, pos_y, "")
        engine.click_position(pos_x, pos_y)
        time.sleep(sleeptime)
    except WeTestRuntimeError as e:
...
def wait_for_scene_danju(name, max_count=20, sleeptime=2):
    """
        等待到达某个场景。如游戏拉起后，可能还需要经历一段开场动画，过一段时间后才进入QQ登录界面，可以使用该接口
    ...
    for i in range(max_count):
        try:
            scene = engine.get_scene()
            str_scene = scene.split('_')
            part_scene = str_scene[0] + str_scene[2]
            print(part_scene)
        except Exception as e:
            logger.error(e)
            time.sleep(sleeptime)
...
def wait_for_scene_game(name, max_count=20, sleeptime=2):
    """
        检测等待场景过程中异常
    ...
    for i in range(max_count):
        try:
            ...
            # 单局过程中的弹框
            err_btn = find_elment_wait("/UIRoot/C_Com_DialogCommon1/Anchor/Button_Mid_Name(Yellow)", 1)
            if err_btn != None:
                screen_shot_click(err_btn)
            screen_shot_click_pos(0, 0)
            time.sleep(sleeptime)
            logger.error(e)
...
def wait_for_scene_level(name, max_count=20, sleeptime=2):
    """
        等待到达某个场景。如游戏拉起后，可能还需要经历一段开场动画，过一段时间后才进入QQ登录界面，可以使用该接口
