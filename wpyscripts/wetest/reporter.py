...
class NativeReporter(Reporter):
    def __init__(self):
        ...
        self.targetpath = os.path.join(os.path.dirname(self.currentpath), "..","..", "testcase", "screenshot")
        if not os.path.isdir(self.targetpath):
            os.makedirs(self.targetpath)

    def screenshot(self):
        name = time.strftime("%Y%m%d%H%M%S", time.localtime())
        cap = "adb shell screencap -p /sdcard/{0}.png".format(name)
        os.system(cap)
        pull = "adb pull /sdcard/{0}.png {1}/{0}.png".format(name,self.targetpath)
        os.system(pull)
        clear = "adb shell rm /sdcard/{0}.png".format(name)
        os.system(clear)
    ...
    def capture_and_mark(self, x, y, locator_name="point"):
        ...
        cap = "adb shell screencap -p /sdcard/{0}.png".format(locator_name)
        os.system(cap)
        pull = "adb pull /sdcard/{0}.png {1}/{0}.png".format(locator_name,self.targetpath)
        os.system(pull)
        clear = "adb shell rm /sdcard/{0}.png".format(locator_name)
        os.system(clear)
