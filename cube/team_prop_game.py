...
from testcase.MyTestAPI import create_motorcade, steal_pig
from wpyscripts.wetest import device
...
def _prepare(test_type):
    ...
    # 每次启动的时候杀掉cubex
    # device.excute_adb("shell am force-stop com.tencent.cubex")
    # device.excute_adb("shell pm clear com.tencent.cubex")
...
def start_steal_pig(test_type):
    ...
    if not prepare:
        ...
        # 没有成功拉起游戏 重新尝试拉起
        team_prop_game.run(test_type)
        # 测试完成
        logger.error("test end bye bye")
        return
    try:
        """此处添加当次测试的内容"""
        ...
        logger.debug("success join in steal pig area...")
        _cube_finish_test()
...
def run(test_type):
    ...
    try:
        ...
        for i in range(2):
            nsss.create_game_room("ECGMT_PROP_TEAM_GAME", 9)
